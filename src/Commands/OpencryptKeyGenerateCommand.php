...
class OpencryptKeyGenerateCommand extends Command
{
    ...
    public function handle()
    {
        ...
        if (config('opencrypt.opencrypt_type') === 'env') {
            if (!$this->setKeyInEnvironmentFile($keyPairs)) {
                return;
            }
        } elseif (config('opencrypt.opencrypt_type') === 'file') {
    ...
    protected function setKeyInEnvironmentFile($keyPairs)
    {
        ...
        if (strlen($currentPrivateKey) !== 0 && strlen($currentPublicKey) !== 0 && (!$this->confirmToProceed())) {
            return false;
        }
    ...
    {
        if (Str::contains(file_get_contents($this->laravel->environmentFilePath()), 'OPENCRYPT_PUBLIC_KEY') === false) {
        ...
        if (Str::contains(file_get_contents($this->laravel->environmentFilePath()), 'OPENCRYPT_PRIVATE_KEY') === false) {
